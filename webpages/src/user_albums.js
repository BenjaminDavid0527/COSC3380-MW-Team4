...
async function get_songs(data) {
    const response = await fetch('/requests/user_songs', {
        method: 'POST',
        headers: {
...
get_songs({UserID: user_id, AlbumID: album_id}).then(results => {
    ...
    for (const song_info of results.Songs) {
        ...
        button.addEventListener('click', () => {
            ...
            rate_song({UserID: user_id, SongID: song_info.id, Rating: rating, WasRated: wasRated, AlbumID: album_id})
            .then( (response) => {
...
get_albums({UserID: user_id}).then(results => {
    const album_list = document.getElementById('song_list');
    const albumTitle = document.getElementById('songTitle');
    // const uploadbtn = document.getElementById('uploadbtn');
    // const deletebtn = document.getElementById('deletebtn');
    // song_info: {id, title, rating}
    for (const album_info of results.albums) {
        const li = document.createElement('li');
        const unique_id = song_info.id.toString();
        li.setAttribute('id', unique_id);
        ...
        button.addEventListener('click', () => {
            ...
            .then( (response) => {
                ...
                if (response.Modified) {
                    wasRated = true;
                    const newBody = `Title: ${song_info.title}\t`
                    const element = document.querySelector('li[id=\'' + unique_id + '\']');
                    element.innerHTML = newBody;
                }
            });
